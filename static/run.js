    ...
    $.get('/getAlbums', {
        ...
        if (data) {
            data.forEach(function(item) {
                $('#resultBox').append("<div class=\"result\" id='" + item.id + "'> <img onerror='this.onerror = null; this.src=\"/static/unknown.jpg\"' src=\"/static/coversLQ/" + item.id + ".jpg\"></img> <h3>" + item.name + "</h3></div>");
           });
        }
    });
}
